...
def _get_dataset():
    ...
    df = pd.read_csv("D:\处理后数据/18-10-24_ok.csv", header=None, names=col_names)
   ...
   #     _encode_text_dummy(df, name)
    ...
    x_train, y_train = IoTData.get_train()#_to_xy(df_train, target='label')
    y_train = y_train.flatten().astype(int)
    x_test, y_test = IoTData.get_test()#_to_xy(df_test, target='label')
...
def _get_adapted_dataset(split):
    ...
    if split != 'train':
        dataset[key_img], dataset[key_lbl] = IoTData._adapt_ratio()#_adapt(dataset[key_img],
                                                    #dataset[key_lbl])
...
scores_pred = clf.decision_function(trainx)
threshold = stats.scoreatpercentile(scores_pred, 100 * outliers_fraction)  #根据训练样本中异常样本比例，得到阈值，用于绘图

# plot the line, the samples, and the nearest vectors to the plane
range_max_min0 = (trainx[:,0].max()-trainx[:,0].min())*0.2
range_max_min1 = (trainx[:,1].max()-trainx[:,1].min())*0.2
xx, yy = np.meshgrid(np.linspace(trainx[:,0].min()-range_max_min0, trainx[:,0].max()+range_max_min0, 500),
                     np.linspace(trainx[:,1].min()-range_max_min1, trainx[:,1].max()+range_max_min1, 500))
...
is_in= clf.predict(trainx)>0
b = plt.scatter(trainx[is_in, 0], trainx[is_in, 1], c='white',
                    s=20, edgecolor='k')
c = plt.scatter(trainx[is_in, 0], trainx[is_in, 1], c='black',
                    s=20, edgecolor='k')
plt.axis('tight')
plt.xlim((-7, 7))
plt.ylim((-7, 7))
