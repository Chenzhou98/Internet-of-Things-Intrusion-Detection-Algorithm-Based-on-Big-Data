...
clf = LocalOutlierFactor(n_neighbors=20, contamination=0.1)#之前n_neighbors=70000 设置样本中异常点的比例，default=0.1
...
for i in range(0, len(testy)):
    if testy[i] == -y_pred[i] and testy[i] == 1:#异常值 预测正确的时候
        posi_num += 1
n_errors = (y_pred-testy!=1).sum()-posi_num  #异常值预测错误#(y_pred != testy).sum()
